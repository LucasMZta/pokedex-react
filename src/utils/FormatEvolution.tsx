...
export const FormatEvolution = ({ data }: Props) => {
    ...
    return (
        <div className="text-sm text-zinc-400 capitalize flex flex-col items-center">
            ...
            <div className="w-0.5 border-r-2 flex-1 mt-2">
                .
            </div>


            {/* {data.trigger.name === 'level-up' &&
                <>
                    {data.min_happiness ? `Happiness ${data.min_happiness}` : `Level ${data.min_level}`}
                </>
            }
            {data.trigger.name === 'use-item' &&
                <>
                    {data.min_happiness ? `Happiness ${data.min_happiness}` : `Level ${data.min_level}`}
                </>
            }
